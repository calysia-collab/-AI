...
階段 2 只能在外部 staging 的必要項目全部通過後開始。
...
11. 執行 `node scripts/audit-data-protection.mjs`。
12. 依 [browser-acceptance.md](browser-acceptance.md) 完成桌面與手機實機簽署。
...
其中 `postgres:test`、`production:preflight`、`staging:clamav`、`staging:endpoint`
必須在對應外部環境變數已設定時執行。
